...
static inline void uart_start(bit_depth_t baud_rate) {
...
static inline bit_depth_t uart_busy() {
    return (MDR_UART2->fr >> 3) & 1;
}

static inline void uart_write(char byte_to_write) {
    while (!uart_busy())
        continue;
    MDR_UART2->dr = (MDR_UART2->dr & ~255) | byte_to_write;
}

static inline bit_depth_t uart_read() { 
    return MDR_UART2->dr & 255;
}

static inline void uart_print(const char *text) {
    for (bit_depth_t i = 0; text[i] != '\0'; ++i)
        uart_write(text[i]);
}

static inline void uart_println(const char *text) {
    uart_print(text);
    uart_write('\n');
}